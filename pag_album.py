...
def panel_adicionar_albuns():
    ...
    nome = StringVar()
    entryNome = Entry(window, width=25, textvariable=nome)
    entryNome.place(x=120, y= 70) 

    lblArtista = Label(window, text = "Artista")
    lblArtista.place(x=70, y=120)
    artista = StringVar()
    entryArtista = Entry(window, width=25, textvariable=artista)
    entryArtista.place(x=120, y= 120) 

    lblAno = Label(window, text = "Ano")
    lblAno.place(x=70, y=170)
    ano = IntVar()
    entryAno = Entry(window, width=25, textvariable=ano)
    entryAno.place(x=120, y= 170) 


    lblGenero = Label(window, text = "Género")
    lblGenero.place(x=70, y=220)
    generoalbum = StringVar()
    generoalbum.set(0)
    rd1 = Radiobutton(window, text = "POP", value = "POP", variable= generoalbum)
    rd2 = Radiobutton(window, text = "HIP-HOP", value = "HIP-HOP", variable= generoalbum)
    rd3 = Radiobutton(window, text = "K-POP", value = "K-POP", variable= generoalbum)
    rd4 = Radiobutton(window, text = "ROCK", value = "ROCK", variable= generoalbum)
    rd5 = Radiobutton(window, text = "R&B", value = "R&B", variable= generoalbum)
    rd6 = Radiobutton(window, text = "COUNTRY", value = "COUNTRY", variable= generoalbum)
    ...
    lblQt = Label(window, text = "Qt músicas")
    lblQt.place(x=370, y=70)
    qt = IntVar()
    entryQt = Entry(window, width=25, textvariable=qt)
    entryQt.place(x=450, y= 70) 

    lblDuracao = Label(window, text = "Duração")
    lblDuracao.place(x=370, y=120)
    duracao = StringVar()
    entryDuracao = Entry(window, width=25, textvariable=duracao)
    entryDuracao.place(x=450, y= 120) 

    lblMetacritic = Label(window, text = "Metacritic")
    lblMetacritic.place(x=370, y=170)
    metacritic = IntVar()
    entryMetacritic = Entry(window, width=25, textvariable=metacritic)
    entryMetacritic.place(x=450, y= 170) 

    lblDescricao = Label(window, text = "Descrição")
    lblDescricao.place(x=370, y=220)
    descricao = StringVar()
    entryDescricao = Entry(window, width=25, textvariable=descricao)
    entryDescricao.place(x=450, y= 220) 

    lblMusicas = Label(window, text = "Músicas")
    lblMusicas.place(x=370, y=270)
    musicas=StringVar()
    entryMusicas = Entry(window, width=25, textvariable=musicas)
